...
router.get('/orders/host/all', ensureAuthenticated, ensureRole('host'), async (req, res) => {
...
router.get('/orders', ensureAuthenticated, ensureRole('customer'), async (req, res) => {
...
router.post('/orders/:id/status', ensureAuthenticated, ensureRole('host'), async (req, res) => {
  const { id } = req.params;
